...
export const usewebinarStore = create<WebinarStore>((set, get) => ({
  ...
  // --- Fixed updateCTAField syntax & types ---
  ...
  // --- Fixed updateAdditionalInfoField syntax & typos ---
  ...
  // --- Fixed validateStep to use correct validators ---
  validateStep: (stepId) => {
    const { formData } = get()
  ...
  getStepValidationErrors: (stepId) => get().validation[stepId].errors,

  resetForm: () => set({ formData: initialState, validation: initialValidation, isComplete: false, isSubmitting: false }),
